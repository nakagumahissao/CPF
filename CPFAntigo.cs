...
namespace CPF
{
    ...
    public class CPFAntigo : ICPF
    {
        ...
        public string EvaluateCPFDV(string cpf9Digits)
        {
            ...
            if (!cpf9Digits.All(char.IsDigit))
            {
        ...
        {
            if (string.IsNullOrEmpty(cpf9Digits + firstDV) || (cpf9Digits + firstDV).Length != 10)
            {
                throw new ArgumentException("CPF must be a string containing exactly 10 digits.");
            }

            // Check if all characters are digits
            if (!((cpf9Digits + firstDV).All(char.IsDigit)))
            {
                throw new ArgumentException("CPF must contain only digits.");
            }
            ...
            for (int i = 0; i < 10; i++)
            {
                sum += ((cpf9Digits + firstDV)[i] - '0') * (11 - i);
            }
